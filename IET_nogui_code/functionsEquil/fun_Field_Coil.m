...
function [ res ] = fun_Field_Coil( source, point, NGauss )
%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
%
...
res_a=   zeros(numel(point.RR),ncoil);
res_br=  zeros(numel(point.RR),ncoil);
res_bz=  zeros(numel(point.RR),ncoil);
res_psi= zeros(numel(point.RR),ncoil);
...
if ncoil<20
    for jj=1:ncoil
        ...
        rr=points_rr;
        % Gauss weights
        [xi,wi] = funGaussLeg(NGauss);
        % Integration of thin solenoid along r
        aphi=zeros(size(points,1),1);
        br=aphi;
        bz=aphi;
        for hh=1:NGauss
            r1=r0+0.5*dr*xi(hh);
            res1=fun_ThinSolenoid(r1,z1,z2,points);% calcolato per corrente unitaria
            aphi= aphi+res1.a*wi(hh)*0.5*source_turns(jj);
            br=   br+res1.br*wi(hh)*0.5*source_turns(jj);
            bz=   bz+res1.bz*wi(hh)*0.5*source_turns(jj);
        end
        res_a(:,jj)=   aphi;
        res_br(:,jj)=  br;
        res_bz(:,jj)=  bz;
        res_psi(:,jj)= aphi.*2*pi.*rr;
    end
else
    parfor jj=1:ncoil
        r0=source_R(jj);
