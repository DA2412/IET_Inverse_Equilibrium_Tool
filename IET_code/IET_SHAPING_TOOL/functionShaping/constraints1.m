...
function F=constraints1(a,delta_u,k_first,csi,s)
    c=csi(1); %csi upper outer 1quadrant
    F=zeros(3,1);
    F(1)=(1-a*(1+delta_u)/s(1))^(s(3)) + (k_first*a/s(2))^s(3) - 1;
    F(2)=(1+a*(1+delta_u) * (1-1/(2^0.5))*(c-1)/s(1))^s(3) + (k_first*a*(1/(sqrt(2))+c*(1-1/(sqrt(2))))/s(2))^s(3)-1;
    if delta_u>=0
        F(3)=(s(2) * ((s(1)-a*(1+delta_u))/(k_first*a))^(s(3)-1))*(s(2)/s(1))^(s(3)-1) - 1/(1-delta_u);
    else
        F(3)=(s(2) * ((s(1)-a*(1+delta_u))/(k_first*a))^(s(3)-1))*(s(2)/s(1))^(s(3)-1) - (1+delta_u);
    end
end