...
function F=constraints3(a,delta_l,k_third,csi,s)
    c=csi(3); %csi lower inner 3quadrant
    F=zeros(3,1);
    F(1)=(1-a*(1-delta_l)/s(1))^(s(3)) + (k_third*a/s(2))^s(3)-1;
    F(2)=(1+a*(1-delta_l)*(1-1/(2^0.5))*(c-1)/s(1))^s(3) + (k_third*a*( 1/(2^0.5) + c*(1-1/(2^0.5))) /s(2))^s(3)-1;
    if delta_l>=0
        F(3)=(s(2)* ( (s(1)-a*(1-delta_l))/(k_third*a) )^(s(3)-1)) * (s(2)/s(1))^(s(3)-1) - (1-delta_l);
    else
        F(3)=(s(2)* ( (s(1)-a*(1-delta_l))/(k_third*a) )^(s(3)-1)) * (s(2)/s(1))^(s(3)-1) - 1/(1+delta_l);
    end
end